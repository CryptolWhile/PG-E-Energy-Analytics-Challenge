...
from utils.visualization import install_packages
...
def load_and_prepare_data(train_path, test_path):
    ...
    - Feature engineering
    - Xử lý NaN
    - Chuẩn bị đầu vào mô hình
    - Kiểm tra test data
    """
    train_df = pd.read_excel(train_path, sheet_name='Data')
    test_df = pd.read_excel(test_path, sheet_name='Data')
    
    # Tạo 2 đặc trưng tuần hoàn (cyclical features):
    # hour_sin, hour_cos giúp mô hình hiểu được tính chu kỳ 24 giờ của biến "Hour".
    # Tránh lỗi “Hour 23 và Hour 0 bị xem là xa nhau” như trong hồi quy tuyến tính thông thường.
    for df in [train_df, test_df]:
        df['hour_sin'] = np.sin(2 * np.pi * df['Hour'] / 24)
        df['hour_cos'] = np.cos(2 * np.pi * df['Hour'] / 24)
    
    # Tạo đặc trưng nhiệt độ trung bình và bức xạ mặt trời trung bình từ 5 site (10 - 2))
    for df in [train_df, test_df]:
        df['avg_temp'] = df[[f'Site-{i} Temp' for i in range(1, 6)]].mean(axis=1)
        df['avg_ghi'] = df[[f'Site-{i} GHI' for i in range(1, 6)]].mean(axis=1)
    
    # 4 Đặc trưng sử dụng cho mô hình
    feature_cols = ['hour_sin', 'hour_cos', 'avg_temp', 'avg_ghi']
    
    # Điền giá trị NaN bằng giá trị trung bình của cột tương ứng
    for df in [train_df, test_df]:
...
def train_xgboost(X_train, y_train):
    """
    Huấn luyện mô hình XGBoost Regressor với 
    ...
    param_grid = {
        'xgb__n_estimators': [100, 200, 300], #số lượng cây boosting.
        'xgb__max_depth': [3, 5, 7], #độ sâu tối đa của mỗi cây.
        'xgb__learning_rate': [0.01, 0.05, 0.1],
        'xgb__subsample': [0.8, 1.0], #tỷ lệ mẫu sử dụng cho mỗi cây.
        'xgb__colsample_bytree': [0.8, 1.0] #tỷ lệ đặc trưng sử dụng cho mỗi cây.giảm overfitting
    }
    
    tscv = TimeSeriesSplit(n_splits=5) # TimeSeriesSplit thay vì KFold
...
def generate_forecast(model, X_test, train_df, test_df, set_name='Test', y_test=None):
    """
    Tạo dự báo và vẽ biểu đồ dựa trên historical data  từ Năm 1 và Năm 2.
    """
    predictions = model.predict(X_test)
